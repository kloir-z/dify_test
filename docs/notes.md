...
- フロー: `start → 各言語の先頭ソース`、先頭ソースが残りの取得ノードをファンアウト → 整形へ合流。
  `国内→整形` と `海外→整形→翻訳→URL復元` を最後の「連結」で待ち合わせて出力
- 合流の待ち合わせ: Dify は複数入力ノードを「全入力到達で発火」する(整形ノードが HTTP 群を待つのと同じ)。
  よって最終「連結」ノードに国内整形と海外URL復元の2本を入れるだけで両ブランチ完了を待てる(IF不要)。
  並列分岐の合流は Variable Aggregator ではなく **Code/Template で行うのが公式の正攻法**(Aggregator は排他分岐用)
- 連結ノードが `## 📰 国内…`(国内側)と `## 🌐 海外…`(海外側)を縦結合し、先頭に `#` タイトルを足す
- **ハマりどころ: 1ノードからの並列分岐は10本まで**(`MAX_PARALLEL_LIMIT` 既定10、Cloud変更不可)。
  当初 `start` から13本(国内5+海外8)出して上限超過 → 「取得後に no output」状態になった。
  対策として言語ごとに「先頭ソースが残りをファンアウトするツリー」にし、どのノードの分岐も ≤10 に抑えた
  (start=2、各言語の先頭=最大8)。→ **`lint_dsl.py` が `(source, sourceHandle)` 単位で分岐数を数え、
  10 超過を ERROR にするようにした**(pre-commit で自動検出)
- **ハマりどころ2: コードノードは「return するキー」と「outputs 宣言」を一致させる**。lint の未使用WARN消し
  目的で宣言だけ削ると、Dify が「宣言外の戻り値=未検証」として `Not all output parameters are validated`
  で落ちる(2ノード異常になった)。宣言を削るなら return 側も削って一致させること(lint も Dify も通る)。
  → **`lint_dsl.py` が `main()` 本体の `return {...}` リテラルを解析し、宣言との差分(宣言外return/未return宣言)
  を ERROR にするようにした**(ヘルパー関数の return は除外。変数 return 等で解析不能なら安全側でスキップ)
- **残課題(ソース障害耐性)**: HTTP ノードは `error_strategy` 未設定なので、1ソースが最終的に失敗すると
  後段の整形ノードが実行されず止まる。各 HTTP ノードの「エラー処理 → デフォルト値(body=空)」を有効にすれば、
  失敗ソースは「(取得失敗または0件)」扱いで継続できる(コード側は空入力を既に許容)

## 2026-06-18 combined を別リポジトリ news_digest の拡張から逆輸入(要約 + URL切替)

- 経緯: combined を Python に移植して育てた別リポジトリ `C:\code\news_digest`(news-digest スキル)で
  2 つ拡張していたので、Dify DSL 側に取り込んだ。手組みだった combined を **ジェネレータ化**(`gen_combined_dsl.py`)
  し、en/security/irodori と同じ「ゼロから dict 構築 → `yaml.dump`」方式に揃えた(エスケープYAMLの手編集を避ける)
- **拡張1: 本文要約**(見にくい3ソース Simon Willison / Nautilus / Aeon に1〜2文の日本語要約を `↳` で差し込み)
  - news_digest 版はサブエージェント(Task)に要約委譲だが、Dify にサブエージェントは無い → **専用「要約」LLM ノード**で実現。
    翻訳ノードは「要約・論評禁止」の厳格プロンプトなので**混ぜず別ノードに分離**(海外整形から翻訳・要約へ2分岐 → URL復元で合流)
  - 海外整形が `summary_src`(`===[[id]]===` / 見出し / 本文 を連結)を出力 → 要約LLMが `[[id]] 要約文` の1行を返す →
    URL復元ノードが `parse_summaries` で `{id:要約}` 化し、本文側 `[[id]]` 行の位置(見出しと日時の間)に `↳` 行を差し込む。
    プレースホルダ id を翻訳・要約・URL の3者で突き合わせる共通キーに使い回す設計
  - **機能差(Dify 制約)**: news_digest.py はフィード本文が薄い(<120字)とき記事ページを補完取得するが、
    **コードノードはネット禁止**(notes 2026-06-12)なので Dify 版は**フィード埋め込み本文だけ**が素材。
    薄い dek のソースは要約が弱くなる/付かない。実フィードで検証したら 30 ブロック取れた(Simon は content 全文、
    Nautilus/Aeon は dek=400〜500字が大半)ので実用上は足りる
- **拡張2: URL 出力の切替**: start の任意入力 `with_urls`(text-input, 既定 `no`)で国内/海外の URL 併記を切替。
  既定は「見出し+日時のみ」(= news_digest の既定)。**旧 combined は常時URL併記だったので既定挙動が変わる**。
  コード側は `(with_urls or "").strip().lower() in ("yes","true","1","on")` で吸収(空=no)
- 並列分岐は維持: start=2 / 日経=5(残り4+整形)/ AP=8(残り7+整形)/ **海外整形=2(翻訳・要約)**。すべて ≤10
- **拡張3(Dify 実行後に追加): 媒体内の重複見出し除去**。Dify で初回実行したら東洋経済等に同一見出しが
  2〜3回出た(Google News RSS が同一記事を複数返す既知の挙動。旧 combined / news_digest.py も未対応だった)。
  各 `parse_rss`/`parse_hn` に正規化タイトル(`" ".join(title.split())` で空白畳み込み)の `seen` 集合を持たせ、
  **媒体内**の完全重複を落とす。媒体をまたぐ同一トピック(日経と朝日の同じ事件等)は別物として残す。
  Atom 分岐は `[:limit]` スライス前に dedup するよう直し、limit が unique 件数で効くようにした
- 検証: `lint_dsl.py` ERROR/WARN なし(ノード21/エッジ33)。生成YAMLからコード文字列を exec し、
  ① 国内 with_urls 切替 ② 海外 summary_src 形式・要約対象外の非混入 ③ URL復元の `↳` 差し込み・URL切替・プレースホルダ消去
  ④ 連結 ⑤ 国内RSS/HN/Atom の媒体内重複除去 をオフライン round-trip で assert。
  `test_dsl_code.py` で国内整形をライブ確認、海外整形も実フィードで summary_src を確認。
  **Dify 実行でも翻訳・要約・既定 no-urls が意図通り動作することを確認済み**(2026-06-18 実行結果)
